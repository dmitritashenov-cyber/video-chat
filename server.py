...
async def websocket_endpoint(ws: WebSocket, room_id: str):
    ...
    # Сообщаем новому клиенту список других клиентов
    existing_clients = [cid for cid in rooms[room_id] if cid != client_id]
    await ws.send_text(json.dumps({"type":"existing","clients":existing_clients}))
        ...
        while True:
            ...
            try:
                ...
                if "chat" in msg_obj:
                    ...
                    for cid2, ws2 in rooms[room_id].items():
                        if cid2 != client_id:
                            await ws2.send_text(json.dumps({"from": client_id, **msg_obj}))
            except:
